...
class StatisticService {
  ...
  // eslint-disable-next-line no-unused-vars
  static async getTransactions(viewProvider, fromClient) {
        ...
        for (const locationBuyRefund of locationBuyRefunds) {
          ...
          if (locationBuyRefund.toParents) {
            if (locationBuyRefund.toParents[viewProvider]) {
              ...
              invoice.newPayload.toParents = locationBuyRefund.toParents;
            }
          }
            ...
            if (!isInPackageRemoves) {
              ...
              if (_package) {
                const buyObject = {
                  ...
                  newRoom: _package.room || locationBuyRefund.room,
                  oldStartDate: null,
                  oldEndDate: null,
                  oldRoom: null,
                ...
                if (
                  ...
                  if (buyObject.startDate < clientPackageTracker[clientId][_packageInfo].endDate) {
                    if (buyObject.newRoom === clientPackageTracker[clientId][_packageInfo].room) {
                      invoice.newPayload.valid = false;
                      buyObject.newStartDate = clientPackageTracker[clientId][_packageInfo].endDate;
                    } else if (buyObject.newRoom < clientPackageTracker[clientId][_packageInfo].room) {
                      buyObject.isRefund = true;
                    } else {
                      buyObject.newStartDate = clientPackageTracker[clientId][_packageInfo].endDate;
                    }
                  }
                  if (
                    buyObject.endDate < clientPackageTracker[clientId][_packageInfo].endDate &&
                    invoice.newPayload.globalAction !== 3
                  ) {
                    if (buyObject.newRoom === clientPackageTracker[clientId][_packageInfo].room) {
                      invoice.newPayload.valid = false;
                ...
                clientPackageTracker[clientId][_packageInfo].room = buyObject.newRoom;
          ...
          for (const _packageRemove of locationBuyRefund.packageRemoves) {
            ...
            const refundObject = {
              ...
              newRoom: _package && _package.room ? _package.room : locationBuyRefund.room,
              oldRoom: null,
              room: locationBuyRefund.room,
            };
            ...
            if (
              ...
              if (refundObject.startDate < clientPackageTracker[clientId][_packageRemove].startDate) {
                if (refundObject.newRoom === clientPackageTracker[clientId][_packageRemove].room) {
                  invoice.newPayload.valid = false;
              ...
              clientPackageTracker[clientId][_packageRemove].room = null;
            } else {
              refundObject.newStartDate = null;
              refundObject.newEndDate = null;
              refundObject.newRoom = null;
              invoice.newPayload.valid = false;
            }
      ...
      if (clientInvoices) {
        for (const clientInvoice of clientInvoices) {
          const invoiceObject = {
            ...
            invoiceProvider: clientInvoice.provider.toString(),
